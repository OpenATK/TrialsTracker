...
function treePut(domain, token, pathString, data, setupTree, headers, websocket) {
	// Walk up the data tree using json pointer. If a particular key occurs where a star exists, then check against `data` to determine whether a resource has been created. 
	let puts = [];
	...
	return Promise.each(tokens, (token, i) => {
		...
		if (pointer.has(setupTree, pointer.compile(setupTokens.slice(0, i)+'/_type'))) {
  		if (!pointer.has(data, pointer.compile(tokens.slice(0, i)+'/_id'))) {
				// Checking against the setupTree requires that we replace the token with a * if a * placeholder exists in the setupTree.
