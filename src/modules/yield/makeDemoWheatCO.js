...
export function makeDemoWheatCO(csvData, offset, connection, id) {
  CONNECTION = connection;
  connection_id = id
  return processData(csvData, offset).then((asHarvested) => {
...
async function processData(data, offset) {
  var asHarvested = {};
    ...
    return asHarvested[crop][geohash].data[id] = pt;
  }).then(() => {
    return pushAsHarvested(asHarvested)
  })
}
